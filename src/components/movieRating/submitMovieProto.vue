	...
	<div :style="$vuetify.breakpoint.width > 600?`margin-top:-52px;`:` `">
		<v-layout justify-center>
			<v-flex xs12 sm10 md8>
				...
				<v-card v-show='!poster_loading' style="height:200px; width:100%;" role='button' v-if='posterId === `` && pre.poster === null'>
				...
				<!-- poster display	 -->
				<div style="position:relative;" v-if='pre.poster || posterId !== ``'>
					...
					<div style='line-height:0'>
						<img :src='posterId?`${$store.state.server}picture/${this.posterId}`:`https://image.tmdb.org/t/p/w500/${pre.poster}`' style="width:100%; height:auto; " >
					</div>
				</div>
				...
				<v-progress-linear v-show='casting_loading' style='margin:0' :height='3' v-bind:indeterminate="true"></v-progress-linear>
					...
					<v-layout wrap >
									...
									<v-container style='height:100%;'  v-bind="{ [`${size}`]: true }">
										<v-card style='max-width:250px;' hover>
											<div class="grey lighten-4" v-ripple :style="`position:relative; line-height:0; overflow:hidden; ${$vuetify.breakpoint.width > 600?`height:250px;`:`height:200px;`}`" >
												...
												<div style="position:absolute; right:0;">
													<v-btn @click='castings?castings:pre.castings' icon small class='white'>
														<v-icon>clear</v-icon>
													</v-btn>
												</div>
												<img style="width:100%;" :src='cast.profileImage?`${$store.state.server}picture/${cast.profileImage}`:`https://image.tmdb.org/t/p/w500/${cast.profile_path}`'>
											</div>
						...
						<template v-for='(cast,i) in pre.castings'>
							<v-slide-x-transition>
								<v-flex xs6 sm4 >
									<v-container style='height:100%;'  v-bind="{ [`${size}`]: true }">
										<v-card style='max-width:250px;' hover>
											<div class="grey lighten-4" v-ripple :style="`position:relative; line-height:0; overflow:hidden; ${$vuetify.breakpoint.width > 600?`height:250px;`:`height:200px;`}`" >
												...
												<div style="position:absolute; right:0;">
													<v-btn @click='pre.castings.splice(i,1)' icon small class='white'>
														<v-icon>clear</v-icon>
													</v-btn>
												</div>
												<img style="width:100%;" :src='cast.profileImage?`${$store.state.server}picture/${cast.profileImage}`:`https://image.tmdb.org/t/p/w500/${cast.profile_path}`'>
	...
	methods:{
		remove(item) {
      	...
      	preview(movie){
      		...
      		this.pre.poster = movie.poster_path;
      		...
      		if(movie.media_type === 'movie'){
      			this.$axios.get(`https://api.themoviedb.org/3/movie/${movie.id}/credits?api_key=${this.$store.state.tmdb}`).then(r=>{
      				...
      				this.pre.castings = r.data.cast.filter(d => d.profile_path !== null);
      				this.casting_loading = false;
      	...
      	errorSnac(){
      		...
      		if(this.pre.poster || this.pre.castings.length < 1){
