...
<v-dialog persistent v-model="dialog" fullscreen transition="dialog-bottom-transition" :overlay='false'>
  <v-card class='black'>
  <div class="center">

<!-- image display centred -->
    <img :src="post.image" class="flexImage" @click='toolbar = !toolbar'/>
  </div>
...
export default{
  ...
  created(){
    ...
    const result = re.test(this.$route.params.id)

    if(result){
      axios.get(`${this.$store.state.server}one/post/${this.$route.params.id}`,this.$store.getters.auth).then(r => {
        this.post = r.data;
        setTimeout(()=>{
        this.dialog = true;
      },100)
    ...
    this.$store.commit('sn',{contaxt:'error',snac:'post id is not valid'});
    setTimeout(()=>{
        this.$router.go(-1);
    },1000)
