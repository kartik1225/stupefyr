<template>
	...
	<div>
		...
		<v-card v-show='loaded'>
			...
			<v-layout class='grey' v-show='!showFilters'>
				<v-btn flat small  class=' white--text grey ' block @click='showFilterFunc'>see all comments</v-btn>
			</v-layout>
			<v-divider></v-divider>
	...
	export default {
		...
		methods:{
			...
			sendReply(cmt){
				...
				this.$axios.post(`${this.$store.state.server}user/commentreply`,data,this.$store.getters.auth).then(r=>{
			...
			sendCommetReply(cmt,par){
				...
				this.$axios.post(`${this.$store.state.server}user/commentreplychild`,data,this.$store.getters.auth).then(r=>{
			...
			sendLike(cmt){
				...
				this.$axios.post(`${this.$store.state.server}user/commentlike`,data,this.$store.getters.auth).then(r=>{
			...
			putCmtEdit(){
				...
				if(this.$store.getters.id === this.cr_edit.user._id){
					...
					this.$axios.put(`${this.$store.state.server}user/newcomment`,this.cr_edit,this.$store.getters.auth).then(r=>{
			...
			confermDelete(){
				...
				if(this.cr_delete.type === 'parent'){
					...
					this.$axios.delete(`${this.$store.state.server}user/deletecmt/${this.cmts[this.cr_delete.index]._id}+${this.$store.getters.id}`,this.$store.getters.auth).then(r=>{
						if(r.data.error){
							this.$store.commit('sn',{context:'error',snac:r.data.error});
						}else{
					...
					this.$axios.delete(`${this.$store.state.server}user/deletecmt/${this.cmts[this.cr_delete.index].replys[this.cr_delete.c_index]._id}+${this.$store.getters.id}`,this.$store.getters.auth).then(r=>{
						if(r.data.error){
							this.$store.commit('sn',{context:'error',snac:r.data.error});
						}else{
			...
			loadMoreReplys(cmt){
				...
				this.$axios.get(`${this.$store.state.server}user/morerep/${cmt._id}+${cmt.replys.length}`,this.$store.getters.auth).then(r=>{
			...
			fetchData(sort){
				...
				this.$axios.get(`${this.$store.state.server}user/sortcmt/${this.post._id}+${this.type}+${this.skip}+${cid}`,this.$store.getters.auth).then(r=>{
					...
					this.$bus.$emit(`post_cmt_loaded`,true)
		...
		mounted(){
			...
			// on new cmt creation....
			this.$bus.$on(`post_cmt_created`,r=>{
